...
object SupabaseDtoMappers {
    ...
    fun mapJsonToRecitation(obj: JSONObject, isLiked: Boolean = false): Recitation {
        ...
        val reciterAvatarRaw = obj.optString("reciter_avatar", obj.optString("avatar_url", ""))
        val reciterAvatar = SupabaseConfig.getPublicAvatarUrl(reciterAvatarRaw)
        val reciterCountry = obj.optString("reciter_country", obj.optString("country", ""))
        ...
        val durationSeconds = obj.optLong("duration_seconds", obj.optLong("duration", 0L))
        val rawAudioUrl = if (obj.isNull("audio_url")) null else obj.optString("audio_url")
        val audioStoragePath = if (obj.isNull("audio_storage_path")) null else obj.optString("audio_storage_path")
        val externalAudioUrl = if (obj.isNull("external_audio_url")) null else obj.optString("external_audio_url")
        val resolvedAudioUrl = SupabaseConfig.resolveAudioUrl(externalAudioUrl, audioStoragePath, rawAudioUrl)
        val coverPath = if (obj.isNull("cover_image_path")) (if (obj.isNull("cover_url")) null else obj.optString("cover_url")) else obj.optString("cover_image_path")
        ...
        val isStaffPick = obj.optBoolean("is_staff_pick", obj.optBoolean("is_featured", false))
